...
$pdf=$_FILES["pdf"]["name"];
	...
	if ($do == "upd") {
		...
		$updtstmt = "UPDATE yhteystiedot SET"
								." otsikko = '".$otsikko."',"
								." teksti = '".$teksti."'"
								." WHERE yhteystiedot_id = ".$yhteystiedot_id;
...
if ($do == "addp") {?>
<p class='pSisalto'><b>Vaihda pdf</b>(Aikaisempi kuva poistetaan automaattisesti palvelimelta kun painetaan Lis&auml;&auml; painiketta ja uusi kuva korvataan edellisell&auml;)</p>
    <?php
        $id=$_REQUEST["id"];
...
if ($do == "add" || $do == "addp") {
    print "<p class='pSisalto'>";
        ...
        print "<input type='hidden' name='kuva' value='".$kuvat["kuva"]."'>";
        print "<input type='hidden' name='kuva' value='".$kuvat["pdf"]."'>";
...
<br/>
	...
	<?php
	$result = mysql_query("SELECT * FROM kuvat WHERE sivu_id=1 OR sivu_id=2 OR sivu_id=3 ORDER BY sivu_id;");
	$rivityyli = "01";
	$rivinro = 1;
	?>
